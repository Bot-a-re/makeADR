...
public class MermaidGenerator {
    ...
    public String generateModuleDiagram(AnalysisResult result) {
        ...
        if (modules.isEmpty()) {
            ...
            for (int i = 0; i < modules.size(); i++) {
                ...
                String nodeId = "M" + i;
                sb.append(String.format("    %s[\"%s<br/>(%d classes)\"]\n", 
                    nodeId, module.getName(), module.getClassCount()));
            }
    ...
    public String generateDataFlowDiagram(AnalysisResult result) {
        ...
        if (!hasApi && !hasService && !hasRepository) {
            ...
            if (hasApi) sb.append("    style API fill:#fff4e6\n");
            if (hasService) sb.append("    style Service fill:#f3e5f5\n");
            if (hasRepository) sb.append("    style Repository fill:#e8f5e9\n");
            if (hasDatabase) sb.append("    style DB fill:#fce4ec\n");
        }
